...
var RenderTokenMock = artifacts.require('RenderTokenMock');
var NewRenderToken = artifacts.require('NewRenderToken');
...
contract('ERC827 RenderToken', function (accounts) {
  let token;

  beforeEach(async function () {
    const oldToken = await RenderTokenMock.new(accounts[0], 400);
    token = await NewRenderToken.new(oldToken.address, 25);
    await oldToken.approve(token.contract.address, 400, {from: accounts[0]});
    await token.migrateBalance({from: accounts[0]});
  });
  ...
  it('should throw an error when trying to transfer more than balance', async function () {
    await token.transfer(accounts[1], 101).should.be.rejectedWith('revert');
  });
  ...
  it('should throw an error when trying to transfer to 0x0', async function () {
    await token.transfer(0x0, 100).should.be.rejectedWith('revert');
  });
